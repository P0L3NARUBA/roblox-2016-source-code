...
int SplitTest(ConvexH& convex, const btPlane& plane) {
	int flag = 0;
	for (size_t i = 0u; i < convex.vertices.size(); i++) {
...
int HullLibrary::calchullgen(btVector3* verts, size_t verts_count, int vlimit)
{
...
int HullLibrary::calchull(btVector3* verts, int verts_count, TUIntArray& tris_out, int& tris_count, int vlimit)
{
	...
	tris_count = ts.size() / 3u;
	tris_out.resize(ts.size());

	for (i = 0u; i < ts.size(); i++)
	{
...
void HullLibrary::BringOutYourDead(const btVector3* verts, unsigned int vcount, btVector3* overts, unsigned int& ocount, unsigned int* indices, unsigned indexcount)
{
	...
	for (i = 0; i < indexcount; i++)
	{
		...
		else
		{
			...
			for (size_t k = 0u; k < m_vertexIndexMapping.size(); k++)
			{
