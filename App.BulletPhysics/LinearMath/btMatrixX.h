...
struct btMatrixX
{
	...
	*/
	...
	{
		int count = 0;
		for (size_t row = 0; row < m_rowNonZeroElements1.size(); row++)
		{
			for (size_t j = 0; j < m_rowNonZeroElements1[row].size(); j++)
			{
...
#else		
		for (size_t i = 0; i < m_colNonZeroElements.size(); i++)
			for (size_t h = 0; h < m_colNonZeroElements[i].size(); h++)
			{
			...
			//if (numZero)
			{
					...
					//for (int g = 0;g<m_colNonZeroElements[j].size();g++)
				{
					...
					else
					{
						...
						if (!useOtherCol)
						{
							for (size_t q = 0; q < other.m_colNonZeroElements[j].size(); q++)
							{
						...
						{
							for (size_t q = 0; q < m_rowNonZeroElements1[i].size(); q++)
							{
