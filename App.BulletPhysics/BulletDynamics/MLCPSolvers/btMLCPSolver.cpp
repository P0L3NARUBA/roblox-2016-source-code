...
btScalar btMLCPSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodiesUnUsed, btPersistentManifold** manifoldPtr, int numManifolds, btTypedConstraint** constraints, int numConstraints, const btContactSolverInfo& infoGlobal, btIDebugDraw* debugDrawer)
{
...

	{
		...
		int dindex = 0;
		for (size_t i = 0; i < m_tmpSolverNonContactConstraintPool.size(); i++)
		{
		...
		{
			for (size_t i = 0; i < m_tmpSolverContactConstraintPool.size(); i++)
			{
		...
		else
		{
			...
			for (size_t i = 0; i < m_tmpSolverContactFrictionConstraintPool.size(); i++)
			{
...
void btMLCPSolver::createMLCPFast(const btContactSolverInfo& infoGlobal)
{
	...
	}
	{
		...
		for (size_t i = 0; i < m_allConstraintArray.size(); i += numRows, c++)
		{
			ofs[c] = rowOffset;
			...
			if (orgBodyA)
			{
				...
				for (size_t row = 0; row < numRows; row++, cur++)
				{
	...
	size_t c = 0;
	{
		...
		for (size_t i = 0; i < m_allConstraintArray.size(); i += numRows, c++)
		{
...

			{
				...
				{
					size_t j0 = jointNodeArray[startJointNodeA].jointIndex;
					size_t cr0 = jointNodeArray[startJointNodeA].constraintRowIndex;
					if (j0 < c)
					{
...

			{
				...
				{
					size_t j1 = jointNodeArray[startJointNodeB].jointIndex;
					size_t cj1 = jointNodeArray[startJointNodeB].constraintRowIndex;
