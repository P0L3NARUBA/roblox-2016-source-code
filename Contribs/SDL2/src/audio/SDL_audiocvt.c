...
SDL_HandTunedTypeCVT(SDL_AudioFormat src_fmt, SDL_AudioFormat dst_fmt)
{
    ...
    return nullptr;                /* no specialized converter code available. */
}
                      ...
                      SDL_AudioFormat src_fmt, SDL_AudioFormat dst_fmt)
{
    if (src_fmt != dst_fmt) {
        ...
        if (filter == nullptr) {
            int i;
            for (i = 0; sdl_audio_type_filters[i].filter != nullptr; i++) {
                          ...
                          int src_rate, int dst_rate)
{
    if (src_rate != dst_rate) {
        ...
        if (filter == nullptr) {
            ...
            for (i = 0; sdl_audio_rate_filters[i].filter != nullptr; i++) {
                  ...
                  SDL_AudioFormat dst_fmt, Uint8 dst_channels, int dst_rate)
{
    ...
    if (cvt->filter_index != 0) {
        ...
        cvt->len = 0;
        cvt->buf = nullptr;
        cvt->filters[cvt->filter_index] = nullptr;
    }
    return (cvt->needed);
}
