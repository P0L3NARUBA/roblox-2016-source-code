...
namespace RBX {
	...
	void CSGMeshSgCore::weldMesh(bool positionOnly)
	{
		...
		for (unsigned int i = 0; i < rawTris.size(); i++)
		{
			...
			{
				if ((rawTris[i].Position - vertices[u].Position).length() < 0.001f)
				{
					if (positionOnly)
					{
						found = true;
	...
	{
		Vector3 normal = (vertB.Position - vertA.Position).cross(vertC.Position - vertA.Position);
	...
	{
		const Vector3& v1 = vertA.Position;
		const Vector3& v2 = vertB.Position;
		const Vector3& v3 = vertC.Position;
	...
	void calcSmoothNormal(CSGVertex& vert, Vector3& normal, const std::vector<CSGVertex>& vertices, std::vector<unsigned int>& indices)
	{
		for (size_t i = 0; i < vertices.size(); i++)
		{
			...
			averageNormal(normal, vert.Position, testVert.Position, vert.Normal, testVert.Normal);
		}
	}
	...
	void calcSmoothTangent(CSGVertex& vert, Vector3& tangent, const std::vector<CSGVertex>& vertices, std::vector<unsigned int>& indices)
	{
		...
		{
			const CSGVertex& testVert = vertices[i];
			averageNormal(tangent, vert.Position, testVert.Position, vert.Tangent, testVert.Tangent);
		}
	}

	void calcUV(CSGVertex& vert)
	{
		Vector3& Pt = vert.Position;
		...
		vert.UV = vert.generateUv(vert.Position);
	}

	void calcFlat(Vector3& normal, unsigned int& uvr,
		CSGVertex& vertA, CSGVertex& vertB, CSGVertex& vertC)
	{
		normal = (vertB.Position - vertA.Position).cross(vertC.Position - vertA.Position);
	...
	bool triangulate3D(sgC3DObject* object, unsigned int& index, std::vector<CSGVertex>& vertices, std::vector<unsigned int>& indices)
	{
		...
		if (triangles)
		{
			...
			{
				CSGVertex vertA;
				vertA.Color = Color4(triangles->allColors[i].x, triangles->allColors[i].y, triangles->allColors[i].z, 1.0f);
				vertA.Position = Vector3(triangles->allVertex[i].x, triangles->allVertex[i].y, triangles->allVertex[i].z);

				CSGVertex vertB;
				vertB.Color = Color4(triangles->allColors[i + 1].x, triangles->allColors[i + 1].y, triangles->allColors[i + 1].z, 1.0f);
				vertB.Position = Vector3(triangles->allVertex[i + 1].x, triangles->allVertex[i + 1].y, triangles->allVertex[i + 1].z);

				CSGVertex vertC;
				vertC.Color = Color4(triangles->allColors[i + 2].x, triangles->allColors[i + 2].y, triangles->allColors[i + 2].z, 1.0f);
				vertC.Position = Vector3(triangles->allVertex[i + 2].x, triangles->allVertex[i + 2].y, triangles->allVertex[i + 2].z);
	...
	bool CSGMeshSgCore::newTriangulate()
	{
		...
		for (auto i = indices.begin(); i != indices.end(); )
		{
			...
			if (i0 == i1 ||
				...
				i2 == i0)
			{
				std::vector<Color4>::iterator c = flatColors.begin();
	...
	void CSGMeshSgCore::buildBRep()
	{
		...
		for (size_t i = 0; i < vertices.size(); i++)
		{
			...
			point.r = vertices[i].Color.r;
			point.g = vertices[i].Color.g;
			point.b = vertices[i].Color.b;
	...
	void CSGClustering::cluster()
	{
		...
		{
			const Vector3& vpos = m_vertices[m_indices[i]].Position;
	...
	void CSGMeshSgCore::makeExtents()
	{
		...
		for (std::vector<CSGVertex>::const_iterator iter = vertices.begin(); iter != vertices.end(); ++iter)
			extents.expandToContain((*iter).Position);
	}
