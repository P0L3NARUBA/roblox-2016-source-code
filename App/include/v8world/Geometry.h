...
namespace RBX {
	...
	public:
		...
		// Grid Size
		virtual void setSize(const G3D::Vector3& _size) { size = _size; }
		const G3D::Vector3& getSize() const { return size; }

		// Parameters
		virtual void setGeometryParameter(const std::string& parameter, size_t value) {
			RBXASSERT(0u);			// stock geometry does not handle parameters.
		}
		virtual size_t getGeometryParameter(const std::string& parameter) const {
			RBXASSERT(0u);			// stock geometry does not handle parameters.
			return 0u;
		}
		...
		virtual size_t getNumSurfaces(void) const = 0;
		virtual Vector3 getSurfaceVertInBody(const size_t surfaceId, const size_t vertId) const = 0;
		virtual size_t getNumVertsInSurface(const size_t surfaceId) const = 0;
		...
		virtual bool collidesWithGroundPlane(const CoordinateFrame& c, float yHeight) const { return ((c.translation.y - getRadius()) < yHeight); }
