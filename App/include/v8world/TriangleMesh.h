...
namespace RBX {
	...
	public:
		TriangleMesh() : version(PHYSICS_SERIAL_VERSION), centerToCornerDistance(0.0f), boundingBoxMesh(nullptr) {
		...
		static bool validateDataVersions(const std::string& data, int32_t& version);
		...
		void updateObjectScale(const std::string& decompKey, const std::string& decompStr, const G3D::Vector3& scale, const G3D::Vector3& meshScale = Vector3(-1.0f, -1.0f, -1.0f));

		// Deserializations
		static std::string generateDecompositionData(size_t numTriangles, const size_t* triangleIndexBase, size_t numVertices, btScalar* vertexBase);
		static std::string generateConvexHullData(size_t numTriangles, const size_t* triangleIndexBase, size_t numVertices, const btVector3* vertexBase);
		static BulletDecompWrapper::ShapeType* retrieveDecomposition(const std::string& str);
		static std::string generateStaticMeshData(const std::vector<size_t>& indices, std::vector<btVector3>& vertices);
		static void readConvexHullData(std::vector<float>& vertices, size_t& numVertices, std::vector<size_t>& indices, size_t& numIndices, btTransform& trans, std::stringstream& stream);
		static void readPrefixData(btVector3& scale, int& currentVersion, std::stringstream& stream);

		// HOUSEKEEPING
		static std::vector<CSGConvex> getDecompConvexes(const std::string& data, int32_t& currentVersion, btVector3& scale, bool dataHasScale = false);
		static void serializeConvexHullData(const btTransform& transform, const size_t numVertices, const float* verticesBase, const size_t numIndices, const size_t* indicesBase, std::stringstream& outstream);

		// Creates decomposition data
		std::string generateDecompositionGeometry(const std::vector<btVector3>& vertices, const std::vector<size_t>& indices);
		...
		size_t getNumSurfaces(void) const { return boundingBoxMesh->getMesh()->numFaces(); }
