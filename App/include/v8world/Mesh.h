		...
		*/
		...
		public:
			...
			const Vertex* getVertex(const Face* face, size_t id) const {
				if (!face || (face == forward)) {
					...
					RBXASSERT(vertex[(id + 1u) % 2u]);

					return vertex[(id + 1u) % 2u];
				}
			}
		...
		public:
			...
			const Vertex* getVertex(size_t id) const {
				return edges[id]->getVertex(this, 0u);
			}

			const Vector3& getVertexOffset(size_t id) const {
			...
			const Plane getSidePlane(size_t edgeId) const {
				...
				return Plane(sideVector, edge->getVertexOffset(this, 0u));
			}

			int32_t getInternalExtrusionIntersection(const Vector3& pBelowInside, const Vector3& pBelowOutside) const;
			int32_t findInternalExtrusionIntersection(const Vector3& p0, const Vector3& p1) const;
			void findInternalExtrusionIntersections(const Vector3& p0, const Vector3& p1, int32_t& side0, int32_t& side1) const;
		...
		private:
			...
			void addFace(size_t numVerts, size_t vertIndexList[], bool reverseOrder);
		...
		public:
			...
			const POLY::Face* getFace(size_t i) const { return &faces[i]; }

			size_t numVertices() const { return vertices.size(); }
			const POLY::Vertex* getVertex(size_t i) const { return &vertices[i]; }

			size_t numEdges() const { return edges.size(); }
			const POLY::Edge* getEdge(size_t i) const { return &edges[i]; }
			...
			void makeVerticalWedgeCell(const Vector3& size, const Vector3& offset, const size_t& orient);
			void makeHorizontalWedgeCell(const Vector3& size, const Vector3& offset, const size_t& orient);
			void makeCornerWedgeCell(const Vector3& size, const Vector3& offset, const size_t& orient);
			void makeInverseCornerWedgeCell(const Vector3& size, const Vector3& offset, const size_t& orient);
		};
	}	// namespace POLY
} // namespace
