...
namespace RBX {
	...
	*/
	...
	Vector3int16 Extents::getCornerIndex(int32_t i) const {
		RBXASSERT(i >= 0);
		RBXASSERT(i <= 7);
		int32_t x_id = i / 4;			// 0,0,0,0,1,1,1,1
		int32_t y_id = (i / 2) % 2;		// 0,0,1,1,0,0,1,1
		int32_t z_id = i % 2;			// 0,1,0,1,0,1,0,1
		return Vector3int16(x_id, y_id, z_id);
	}

	Vector3 Extents::getCorner(int32_t i) const {
	...
	Vector3 Extents::faceCenter(NormalId faceId) const {
		...
		int32_t xyz = faceId % 3;

		if (faceId < 3) // i.e., positive face x, y, z
			answer[xyz] = high[xyz];
		else // i.e., negative face -x, -y, -z
			answer[xyz] = low[xyz];
	...
	float Extents::computeClosestSqDistanceToPoint(const Vector3& point) const {
		Vector3 center = (high + low) / 2.0f;
		Vector3 extents = (high - low) / 2.0f;
