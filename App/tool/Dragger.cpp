...
namespace RBX {
	...
	*/
		...
		const float customPlaneHeight)
	{
		...
		if (primitives.size() > (size_t)RBX::ClientAppSettings::singleton().GetValueMinPartsForOptDragging())
		...
		const Vector3& movedSoFar)
	{
		...
		for (size_t a = 0u; a < primExtents.size(); ++a) {
			//look for only 1 prim instead of getting a complete list
			foundPrims.fastClear();
			contactManager.getPrimitivesTouchingExtents(primExtents[a], ignorePrimitives, 1u, foundPrims);

			if (foundPrims.size()) {
				//ideally we must get only one prim here!!!
				for (size_t zz = 0u; zz < foundPrims.size(); ++zz) {
					RBX::Extents tmpExtent(Math::toGrid(foundPrims[zz]->getFastFuzzyExtents().min(), Tolerance::mainGrid()), Math::toGrid(foundPrims[zz]->getFastFuzzyExtents().max(), Tolerance::mainGrid()));
					tmpExtent.expand(-Tolerance::maxOverlapOrGap() + 0.001f);

					if (tmpExtent.overlapsOrTouches(primExtents[a])) {
						//if the found prim is a block and not rotated then return
						if (foundPrims[zz]->getConstGeometry()->getGeometryType() == Geometry::GEOMETRY_BLOCK && Math::isAxisAligned(foundPrims[zz]->getCoordinateFrame().rotation))
							return true;
