	...
	*/
	BulletShapeConnector::~BulletShapeConnector() {
		if (bulletAlgo) {
			...
			for (size_t i = 0u; i < manArray[bulletManifoldIndex]->getNumContacts(); i++) {
	...
	void BulletShapeConnector::updateConnectorPointFromManifold(bool refreshContacts) {
		...
		if (hasValidPoints) {
			...
			contactPoint.length = conPoint.getDistance() < 0.0f ? -contactPoint.normal.unitize() : contactPoint.normal.unitize();
		}
		else
			contactPoint.length = 1.0f;
	}
	...
	void BulletShapeConnector::realignConnectorsToBulletContacts() {
		...
		for (size_t i = 0; i < manifoldArray[bulletManifoldIndex]->getNumContacts(); i++) {
