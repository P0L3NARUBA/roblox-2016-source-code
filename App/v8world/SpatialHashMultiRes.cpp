...
namespace RBX {
	...
	void SpatialHashStatic::makeVisitOrder(int32_t* offsets, const Vector3& visitDir) {
	...
	*/
	...
	int32_t SpatialHashStatic::getHash(int32_t level, const Vector3int32& grid) {
		uint32_t x = grid.x & 0xFFFFFFFF;
		uint32_t y = grid.y & 0xFFFFFFFF;
		uint32_t z = grid.z & 0xFFFFFFFF;

		uint32_t xProd = x * 73856093;
		uint32_t yProd = y * 19349663;
		uint32_t zProd = z * 83492791;

		int32_t hash = xProd ^ yProd ^ zProd;

		return hash & (numBuckets(level) - 1u);
	}

	void SpatialHashStatic::computeMinMax(const int32_t level, const Extents& extents, Vector3int32& min, Vector3int32& max) {
		RBXASSERT(level != -1);
