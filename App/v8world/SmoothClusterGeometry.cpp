...
static const size_t SmoothTerrainPhysicsCacheSize = 16u * 1024u * 1024u;
...
namespace RBX {
	...
	struct ChunkMeshShapeData {
		...
		ChunkMeshShapeData(const Voxel2::Mesher::BasicMesh& geometry)
		{
			...
			size_t triangleCount = geometry.indices.size() / 3u;
			size_t waterTriangles = 0u;

			std::vector<char> iswater(triangleCount);

			for (size_t i = 0u; i < triangleCount; ++i) {
				const Vertex& v0 = geometry.vertices[geometry.indices[3u * i]];
				const Vertex& v1 = geometry.vertices[geometry.indices[3u * i + 1u]];
				const Vertex& v2 = geometry.vertices[geometry.indices[3u * i + 2u]];
			...
			solidIndices.reset(new size_t[solidTriangleCount * 3u]);

			waterTriangleCount = waterTriangles;
			waterIndices.reset(new size_t[waterTriangleCount * 3u]);

			size_t solidOffset = 0u;
			size_t waterOffset = 0u;

			for (size_t i = 0u; i < triangleCount; ++i) {
				size_t i0 = geometry.indices[3u * i];
				size_t i1 = geometry.indices[3u * i + 1u];
				size_t i2 = geometry.indices[3u * i + 2u];

				if (iswater[i]) {
					waterIndices[waterOffset] = i0;
					waterIndices[waterOffset + 1u] = i1;
					waterIndices[waterOffset + 2u] = i2;

					waterOffset += 3u;
				}
				else {
					solidIndices[solidOffset] = i0;
					solidIndices[solidOffset + 1u] = i1;
					solidIndices[solidOffset + 2u] = i2;
	...
	public:
		...
		PartMaterial getTriangleMaterial(uint32_t triangleIndex, const Vector3& localHitPoint) {
		...
		const btVector3& getLocalScaling() const override {
			static const btVector3 result(1.0f, 1.0f, 1.0f);
		...
		size_t getMemorySize() const {
			...
			result += data.solidTriangleCount * 3u * sizeof(data.solidIndices[0]);
			result += data.waterTriangleCount * 3u * sizeof(data.waterIndices[0]);
	...
	struct SmoothClusterGeometry::ChunkMesh {
		...
		Voxel2::Region getRegion() const {
			return Voxel2::Region::fromChunk(id, TerrainPartitionSmooth::kChunkSizeLog2).expand(1u);
		}
		...
		void generateShape(MegaClusterInstance* mci) {
			...
			BasicMesh geometry = generateGeometry(box, region.begin(), 0u, options);
	...
	size_t SmoothClusterGeometry::getNumSurfaces() const {
		return 0u;
	}

	Vector3 SmoothClusterGeometry::getSurfaceVertInBody(const size_t surfaceId, const size_t vertId) const {
		return Vector3();
	}

	size_t SmoothClusterGeometry::getNumVertsInSurface(const size_t surfaceId) const {
		return 0u;
	}
	...
	bool SmoothClusterGeometry::hitTestTerrain(const RbxRay& rayInMe, Vector3& localHitPoint, int32_t& surfId, CoordinateFrame& surfCf) {
		Vector3 normal;
		uint8_t material;
	...
	struct DbvhRayTest {
		...
		void processTree(const btDbvtNode* root) {
			...
			rayDirectionInverse[0] = rayDir[0] == btScalar(0.0f) ? btScalar(BT_LARGE_FLOAT) : btScalar(1.0f) / rayDir[0];
			rayDirectionInverse[1] = rayDir[1] == btScalar(0.0f) ? btScalar(BT_LARGE_FLOAT) : btScalar(1.0f) / rayDir[1];
			rayDirectionInverse[2] = rayDir[2] == btScalar(0.0f) ? btScalar(BT_LARGE_FLOAT) : btScalar(1.0f) / rayDir[2];
			size_t raySigns[3] = { rayDirectionInverse[0] < 0.0f, rayDirectionInverse[1] < 0.0f, rayDirectionInverse[2] < 0.0f };

			btAlignedObjectArray<DbvhNode> stack;
			size_t stackOffset = 0u;

			stack.resize(btDbvt::DOUBLE_STACKSIZE);

			stack[stackOffset++] = DbvhNode(root, 0.0f);

			btVector3 bounds[2];

			while (stackOffset > 0u) {
				...
				if (node.node->isinternal()) {
					...
					hit[0] = btRayAabb2(rayOrigin, rayDirectionInverse, raySigns, bounds, tmin[0], 0.0f, 1.0f);

					bounds[0] = node.node->childs[1]->volume.Mins();
					bounds[1] = node.node->childs[1]->volume.Maxs();
					hit[1] = btRayAabb2(rayOrigin, rayDirectionInverse, raySigns, bounds, tmin[1], 0.0f, 1.0f);

					if (stackOffset + 2u > stack.size())
						stack.resize(stack.size() * 2u);

					// start with the node that's closer to the ray origin
					uint8_t i0 = (tmin[0] < tmin[1]) ? 0u : 1u;
					uint8_t i1 = 1u - i0;
	...
	bool SmoothClusterGeometry::castRay(const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal, uint8_t& surfaceMaterial, float maxDistance, bool ignoreWater) {
		...
		for (size_t i = 0u; i < test.updatedChunks.size(); ++i)
	...
	void SmoothClusterGeometry::updateAllChunks() {
		...
		for (size_t i = 0u; i < regions.size(); ++i) {
			std::vector<Vector3int32> chunks = regions[i].expand(1u).getChunkIds(TerrainPartitionSmooth::kChunkSizeLog2);
	...
	void SmoothClusterGeometry::garbageCollectIncremental() {
		...
		size_t visitCount = std::min(bulletChunks.size(), allocatedCount + 64u);
		...
		for (size_t j = 0u; j < visitCount; ++j) {
			...
			if (mesh->shape && mesh->shape.unique()) {
				...
				if (gcUnusedMemory > SmoothTerrainPhysicsCacheSize) {
	...
	PartMaterial SmoothClusterGeometry::getTriangleMaterial(btCollisionShape* collisionShape, uint32_t triangleIndex, const Vector3& localHitPoint) {
