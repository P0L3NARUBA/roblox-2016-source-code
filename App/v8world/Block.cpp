...
namespace RBX {
	...
	*/
	...
	const uint32_t Block::BLOCK_FACE_TO_VERTEX[6][4] = {
		0u, 2u, 3u, 1u,	// x+
		0u, 1u, 5u, 4u,	// y+
		0u, 4u, 6u, 2u,	// z+
		4u, 5u, 7u, 6u,	// x-
	...
	*/
	...
	const uint32_t Block::BLOCK_FACE_VERTEX_TO_EDGE[6][4] = {
		4u, 11u, 5u,  8u,
		8u, 3u,  9u,  0u,
		0u, 7u,  1u,  4u,
		9u, 6u,  10u, 7u,
	...
	*/
	...
	bool Block::hitTest(const RbxRay& rayInMe, Vector3& localHitPoint, Vector3& surfaceNormal) {
		Vector3 halfRealSize = getSize() * 0.5f;
	...
	const Vector3* Block::getCornerPoint(const Vector3int16& clip) const {
		uint8_t x = clip.x > 0u ? 0u : 1u;
		uint8_t y = clip.y > 0u ? 0u : 1u;
		uint8_t z = clip.z > 0u ? 0u : 1u;

		return &vertices[x * 4u + y * 2u + z];
	}
	...
	const Vector3* Block::getEdgePoint(const Vector3int16& clip, RBX::NormalId& normalID) const {
		...
		if (clip.x == 0) {
			...
			uint8_t y = clip.y > 0 ? 0u : 1u;
			uint8_t z = clip.z > 0 ? 0u : 1u;

			return &vertices[4u + y * 2u + z];
		}

		if (clip.y == 0) {
			normalID = RBX::NORM_Y_POS;

			uint8_t x = clip.x > 0 ? 0u : 1u;
			uint8_t z = clip.z > 0 ? 0u : 1u;

			return &vertices[x * 4u + 2u + z];
		}
	...
	const Vector3* Block::getPlanePoint(const Vector3int16& clip, RBX::NormalId& normalID) const {
		...
		if (clip.x != 0) {
			...
			return &vertices[x * 4u];		// either +x,+y,+z or -x,+y,+z
		}
	...
	GeoPairType Block::getBallBlockInfo(uint32_t onBorder, const Vector3int16 clip, const Vector3*& offset,
	...
	GeoPairType Block::getBallInsideInfo(const Vector3& ray, const Vector3*& offset, RBX::NormalId& normalID) {
		...
		for (size_t i = 0u; i < 3u; i++) {
			...
			if (temp < min) {
				...
				normalID = static_cast<RBX::NormalId>(i + 3u);
			}
		}
	...
	void Block::projectToFace(Vector3& ray, Vector3int16& clip, uint32_t& onBorder) {
		onBorder = 0u;
		const Vector3& l = vertices[0];		// all positive;
	...
	uint32_t Block::getClosestEdge(const Matrix3& rotation, NormalId normalID, const Vector3& crossAxis) {
		Vector3 axisInBody = Math::vectorToObjectSpace(crossAxis, rotation);

		Vector2 projected = getProjectedVertex(axisInBody, normalID);

		if (projected.y > 0.0f) {
			if (projected.x > 0.0f) { return normalID * 4u; }
			else { return normalID * 4u + 1u; }
		}
		else {
			if (projected.x > 0.0f) { return normalID * 4u + 3u; }
			else { return normalID * 4u + 2u; }
		}
	}
	...
	CoordinateFrame Block::getSurfaceCoordInBody(const size_t surfaceId) const {
		...
		aCS.translation = mesh->getFace(surfaceId)->getVertex(2u)->getOffset();
