...
#pragma warning(pop)
	...
	size_t Primitive::getSizeMultiplier() const {
		switch (sizeMultiplier) {
		case Primitive::DEFAULT_SIZE:	return 1u;
		case Primitive::TORSO_SIZE:		return 5u;
		case Primitive::ROOT_SIZE:		return 10u;
		case Primitive::SEAT_SIZE:		return 20u;
	...
	void Primitive::setSizeMultiplier(SizeMultiplier value) {
		if (value != sizeMultiplier) {
			if (!world) {
				...
				RBXASSERT(0u);		// show to dave - changing primitive name to/from "Torso" while in world
			}
		}
	}
	...
	Primitive* Primitive::downstreamPrimitive(Joint* j) {
		Primitive* p0 = j->getPrimitive(0u);
		Primitive* p1 = j->getPrimitive(1u);
	...
	Edge* EdgeList::getNext(const Primitive* p, Edge* e) const {
		...
		size_t nextIndex = (size_t)e->getIndex(p) + 1u;
		return (nextIndex < list.size()) ? list[nextIndex].edge : nullptr;
	}
	...
	void EdgeList::removeEdge(Edge* e) {
		int32_t removeIndex = e->getIndex(owner);
	...
	Joint* Primitive::getJoint(size_t id) {
		return rbx_static_cast<Joint*>(joints.getEdge(id));
	}

	const Joint* Primitive::getConstJoint(size_t id) const {
		return rbx_static_cast<Joint*>(joints.getEdge(id));
	}


	Contact* Primitive::getContact(size_t id) {
		return rbx_static_cast<Contact*>(contacts.getEdge(id));
	}

	void Primitive::insertEdge(Edge* e) {
		Primitive* p0 = e->getPrimitive(0u);
		Primitive* p1 = e->getPrimitive(1u);
	...
	Edge* Primitive::getFirstEdge() const {
		return (joints.size() > 0u) ? joints.getEdge(0u) : contacts.getFirst();
	}
	...
	Joint* Primitive::getJoint(Primitive* p0, Primitive* p1, size_t index) {
		...
		size_t foundId = 0u;
		for (size_t i = 0u; i < leastJoints->getNumJoints(); ++i) {
	...
	Contact* Primitive::getContact(Primitive* p0, Primitive* p1) {
		...
		for (size_t i = 0u; i < leastContacts->getNumContacts(); ++i) {
	...
	size_t Primitive::getGeometryParameter(const std::string& parameter) const {
	...
	void Primitive::setGeometryParameter(const std::string& parameter, size_t value) {
	...
	float Primitive::computeJointK() {
		Vector3 size = getGeometry()->isTerrain() ? Vector3(4.0f, 4.0f, 4.0f) : getSize();
	...
	void Primitive::calculateSortSize() {
		...
		RBXASSERT(planar * 1000.0f + 1.0f < (float)std::numeric_limits<size_t>::max());
		RBXASSERT(planar > 0.0f);

		//unsigned long long int planarInt = Math::iFloor(planar);
		size_t planarInt = Math::iFloor(planar * 50.0f);
		size_t multiplier = getSizeMultiplier();

		sortSize = planarInt * multiplier + 1u; // we reserve 0 for uninitialized parts
	}
