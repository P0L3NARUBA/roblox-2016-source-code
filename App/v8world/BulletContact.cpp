...
namespace RBX {
	...
	static void matchConnectors(World* world, btManifoldArray& manifoldArray, BulletConnectorArray& connectors) {
		...
		for (size_t i = 0u; i < connectors.size(); ) {
			BulletConnector* conn = connectors[i];
	...
	static void createConnectors(btManifoldArray& manifoldArray, BulletConnectorArray& connectors, Body* body0, Body* body1, ContactParams* contactParams) {
		for (size_t i = 0u; i < manifoldArray.size(); ++i) {
			...
			for (size_t j = 0u; j < manifold->getNumContacts(); ++j) {
				...
				if (!pt.m_userPersistentData && pt.getDistance() < 0.0f) {
	...
	ContactConnector* BulletContact::getConnector(size_t i) {
		return connectors[i];
	}

	size_t BulletContact::numConnectors() const {
	...
	bool BulletContact::computeIsColliding(float overlapIgnored) {
		Primitive* prim0 = getPrimitive(0u);
		Primitive* prim1 = getPrimitive(1u);
	...
	ContactConnector* BulletCellContact::getConnector(size_t i) {
		return connectors[i];
	}

	size_t BulletCellContact::numConnectors() const {
		return connectors.size();
	}
	...
	void BulletCellContact::updateContactParemeters(btCollisionObject* cellObj) {
		...
		for (size_t i = 0u; i < connectors.size(); ++i) {
			...
			size_t triangleIndex = manifold->getBody0() == cellObj ? pt.m_index0 : pt.m_index1;

			Primitive* terrainPrimitive = getPrimitive(0u);
			if (DFFlag::UseTerrainCustomPhysicalProperties && terrainPrimitive->getPhysicalProperties().getCustomEnabled()) {
				MaterialProperties::updateContactParamsPrims(newParams, getPrimitive(1u), terrainPrimitive);
				...
				MaterialProperties::updateContactParamsPrimMaterial(newParams, getPrimitive(1u), terrainPrimitive, terrainContactMaterial);
				conn->setContactParams(newParams);
			}
		}
	}
