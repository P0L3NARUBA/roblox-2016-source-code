...
namespace RBX {
	...
	void PrismPoly::setGeometryParameter(const std::string& parameter, size_t value) {
	...
	size_t PrismPoly::getGeometryParameter(const std::string& parameter) const {
		if (parameter == "NumSides") {
			return numSides;
		}
		else if (parameter == "NumSlices") {
	...
	void PrismPoly::setNumSides(size_t num) {
		if (numSides != num) {
			numSides = num;
			buildMesh();
		}
	...
	void PrismPoly::setNumSlices(size_t num) {
		if (numSlices != num) {
			numSlices = num;
			buildMesh();
		}
	...
	CoordinateFrame PrismPoly::getSurfaceCoordInBody(const size_t surfaceId) const {
		...
		if (surfaceId == mesh->numFaces() - 1u || surfaceId == mesh->numFaces() - 2u) {
			...
			mesh->getFace(mesh->numFaces() - 1u)->getOrientedBoundingBox(xF, yF, bMin, bMax, bCenter);
