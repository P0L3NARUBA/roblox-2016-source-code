...
namespace RBX {
	...
	Matrix3 Cylinder::getMoment(float mass) const {
		...
		float x = (1.0f / 2.0f) * mass * radius2;
		float yz = (1.0f / 4.0f) * mass * radius2 + (1.0f / 12.0f) * mass * length2;
	...
	size_t Cylinder::getNumSurfaces() const {
		return 6u;
	}
		...
		if (DFFlag::CylinderSurfaceNormalHitFix) {
			...
			if (rayInMe.origin().x < -halfSize.x) {
				if (rayInMe.direction().x > 0.0f) {
					...
					double distance = (intersectionPoint - Vector3(-halfSize.x, 0.0f, 0.0f)).magnitude();
			...
			Vector3 projectedDirection = rayInMe.direction() * Vector3(0.0f, 1.0f, 1.0f);
			Vector3 projectedOrigin = rayInMe.origin() * Vector3(0.0f, 1.0f, 1.0f);

			double a = projectedDirection.dot(projectedDirection);
			double b = 2.0 * projectedOrigin.dot(projectedDirection);
			...
			if (distance > 0.0) {
				...
				if (intersectionPoint.x < halfSize.x && intersectionPoint.x > -halfSize.x) {
					surfaceNormal = (intersectionPoint * Vector3(0.0f, 1.0f, 1.0f)).unit();
					localHitPoint = intersectionPoint;
			...
			btCollisionObjectWrapper colObWrap(nullptr, bulletCylinderShape->getShape(), getBulletCollisionObject(), identityTransform, -1, -1);
	...
	Vector3 Cylinder::getSurfaceVertInBody(const size_t surfaceId, const size_t vertId) const {
		Vector3 size(realLength, realWidth, realWidth);
	...
	size_t Cylinder::getNumVertsInSurface(const size_t surfaceId) const {
		return 4u;
	}
	...
	CoordinateFrame Cylinder::getSurfaceCoordInBody(const size_t surfaceId) const {
		float sign = (surfaceId >= NORM_X_NEG) ? -1.0f : 1.0f;

		Vector3 faceCentroid;
		faceCentroid.x = (surfaceId == NORM_X_POS || surfaceId == NORM_X_NEG) ? sign * realLength * 0.5f : 0.0f;
		faceCentroid.y = (surfaceId == NORM_Y_POS || surfaceId == NORM_Y_NEG) ? sign * realWidth * 0.5f : 0.0f;
		faceCentroid.z = (surfaceId == NORM_Z_POS || surfaceId == NORM_Z_NEG) ? sign * realWidth * 0.5f : 0.0f;
