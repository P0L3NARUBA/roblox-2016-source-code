...
namespace RBX
{
	...
	namespace Reflection {
		template<>
		EnumDesc<CollisionFidelity>::EnumDesc() :EnumDescriptor("CollisionFidelity") {
	...
	shared_ptr<CSGMesh> PartOperation::getMesh() {
		if (!cachedMesh) {
			...
			if ((cachedMesh->isBadMesh() || cachedMesh->getIndices().size() / 3u > getMaximumTriangleCount()) && Workspace::serverIsPresent(this))
		...
		if (FFlag::CSGPhysicsLevelOfDetailEnabled) {
			...
			try {
				...
				if (getCollisionFidelity() == CollisionFidelity_Default) {
					...
					const std::vector<uint32_t> indices = mData->getIndices();

					if (vertices.size() > 0u && indices.size() > 0u) {
						decompBinString = BinaryString(TriangleMesh::generateDecompositionData(indices.size() / 3u, &indices[0], vertices.size(), (btScalar*)&vertices[0].x()));
					...
					const std::vector<uint32_t> indices = mData->getIndices();
					if (vertices.size() > 0u && indices.size() > 0u) {
						decompBinString = BinaryString(TriangleMesh::generateConvexHullData(indices.size() / 3u, &indices[0], vertices.size(), &vertices[0]));
						getPrimitive(this)->resetGeometryType(Geometry::GEOMETRY_TRI_MESH);
					}
					else {
						decompBinString = BinaryString();
	...
	Vector3 PartOperation::calculateAdjustedSizeDifference(const Vector3& newBoundingBoxSize) {
		Vector3 result = calculateSizeDifference(newBoundingBoxSize) - ((Vector3(2.0f * bulletCollisionMargin)) / initialSize);
	...
	std::vector<btVector3> getCSGVertexPositions(const std::vector<CSGVertex>& vertices) {
		...
		for (size_t i = 0u; i < vertices.size(); i++) {
			outputVec.push_back(btVector3(vertices[i].Position.x, vertices[i].Position.y, vertices[i].Position.z));
		}
