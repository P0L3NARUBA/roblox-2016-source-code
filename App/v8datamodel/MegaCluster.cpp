...
static const size_t SmoothTerrainMaxLuaRegion = 4u * 1024u * 1024u;
static const size_t SmoothTerrainMaxCppRegion = 64u * 1024u * 1024u;
...
namespace RBX {
	...
	namespace Reflection {
		...
		template<>
		CellMaterial& Variant::convert<CellMaterial>(void) { return genericConvert<CellMaterial>(); }
		...
		template<>
		CellBlock& Variant::convert<CellBlock>(void) { return genericConvert<CellBlock>(); }
		...
		template<>
		CellOrientation& Variant::convert<CellOrientation>(void) { return genericConvert<CellOrientation>(); }
		...
		template<>
		WaterCellForce& Variant::convert<WaterCellForce>(void) { return genericConvert<WaterCellForce>(); }
		...
		template<>
		WaterCellDirection& Variant::convert<WaterCellDirection>(void) { return genericConvert<WaterCellDirection>(); }
	}
	...
	const float MegaClusterInstance::kConstReflectance = 0.0f;
	const Vector3 MegaClusterInstance::kConstRotVelocity(0.0f, 0.0f, 0.0f);
	const float MegaClusterInstance::kConstTransparency = 0.0f;
	const Vector3 MegaClusterInstance::kConstVelocity(0.0f, 0.0f, 0.0f);
	...
	MegaClusterInstance::MegaClusterInstance()
		: smoothReplicate(-1)
		, waterColor(1.0f, 1.0f, 1.0f)
		, waterTransparency(1.0f)
		, waterWaveSize(0.15f)
		, waterWaveSpeed(10.0f)
	{
		FASTLOG1(FLog::MegaClusterInit, "MegaCluster created - %p", this);

		Vector3 terrainSize = Vector3(511.0f * 4.0f, 63.0f * 4.0f, 511.0f * 4.0f);
		setName("Terrain");
		...
		materialTable.reset(new Voxel2::MaterialTable(ContentProvider::findAsset(kSmoothTerrainMaterials), Voxel2::Cell::Material_Max + 1u));
	...
	MegaClusterInstance::~MegaClusterInstance() {
		...
		if (parentWorkspace)
			parentWorkspace->setTerrain(nullptr);
	...
	void MegaClusterInstance::verifySetParent(const Instance* instance) const {
		...
		const Workspace* workspace = instance != nullptr ? instance->fastDynamicCast<Workspace>() : nullptr;
	...
	void MegaClusterInstance::reloadMaterialTable()
	{
		...
		for (size_t i = 0u; i < myPrim->getNumContacts(); ++i) {
	...
	static Voxel2::Region getRegionAtResolution(const Region3& region, float resolution)
	{
		...
		if (result.getChunkCount(0) > SmoothTerrainMaxLuaRegion)
	...
	static Voxel2::Region getRegionFromExtents(const Vector3& min, const Vector3& max)
	{
		...
		if (result.getChunkCount(0) > SmoothTerrainMaxCppRegion)
