...
#undef LOOKUP
		...
		template<bool vertical> void GeometryGenerator::addCylinder(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed, bool ignoreMaterialsStuds)
		{
			...
			float surfaceTiling = decal ? 1 : getSurfaceTiling(renderMaterial);
		...
		void GeometryGenerator::addBlock(const Vector3& size, const Vector3& center, PartInstance* part, Decal* decal, const Options& options, unsigned int randomSeed, bool ignoreMaterialsStuds)
		{
			...
			if (decal)
			{
				...
				fillVertex(vertices[vertexOffset + 0], corners[faces[face][0]], Vector2(uv.x, 0.0f), color, normals[face], Vector3());
				fillVertex(vertices[vertexOffset + 1], corners[faces[face][1]], Vector2(0.0f, 0.0f), color, normals[face], Vector3());
				fillVertex(vertices[vertexOffset + 2], corners[faces[face][2]], Vector2(uv.x, uv.y), color, normals[face], Vector3());
				fillVertex(vertices[vertexOffset + 3], corners[faces[face][3]], Vector2(0.0f, uv.y), color, normals[face], Vector3());
