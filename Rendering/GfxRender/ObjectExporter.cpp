	...
	namespace Graphics
	{
		...
		void ObjectExporter::processPart(PartInstance* part, Decal* decal) {
			...
			if (count.getVertexCount() > 0 && count.getIndexCount() >= 3)
			{
				...
				options = GeometryGenerator::Options(visualEngine, *part, decal, part->getPart().coordinateFrame, materialResult.flags, materialResult.uvOffsetScale);
				...
				GeometryGenerator generator(&model.vertices[0], &model.indices[0], 0u);
				...
				Color4 vertColor = model.vertices[0].Color;
				...
				if (decal)
					for (size_t i = 0u; i < model.vertices.size(); ++i) {
		...
		AABox ObjectExporter::computeAABB() {
			...
			for (Models::iterator it = models.begin(); it != models.end(); ++it) {
				for (size_t i = 0u; i < it->vertices.size(); ++i)
		...
		bool ObjectExporter::getImage(Texture* tex, G3D::GImage& imgOut) {
			...
			uint32_t length = Texture::getImageSize(format, w, h);

			imgOut = G3D::GImage(w, h, 4u);
			...
			case Texture::Format_BC3: {
				...
				if (!tex->download(0u, 0u, data.get(), length))
					return false;

				Image::decodeDXT(imgOut.byte(), data.get(), w, h, 1u, format);

				break;
			}
			case Texture::Format_RGBA8: {
				if (!tex->download(0u, 0u, imgOut.byte(), length))
					return false;

				// in this case we have to swap to RGB
				if (visualEngine->getDevice()->getCaps().colorOrderBGR) {
					uint8_t* data = imgOut.byte();
					for (size_t i = 0u; i < length / 4u; ++i) {
						size_t idx = i * 4u;
						std::swap(data[idx], data[idx + 2u]);
					}
				}
		...
		void ObjectExporterObj::exportObj(std::ostream& exportStream, bool encodeBase64 /* = false */) {
			uint32_t vertexOffset = 1u; // obj vertices starts at index 
			...
			if (visualEngine->getSettings()->getObjExportMergeByMaterial()) {
				for (MatarialSet::iterator it = materialMap.begin(); it != materialMap.end(); ++it) {
					...
					// Verts
					for (size_t i = 0u; i < mat.models.size(); ++i) {
					...
					for (size_t i = 0u; i < mat.models.size(); ++i) {
						...
						for (size_t i = 0u; i < model->indices.size(); i += 3u)
				...
				for (Models::iterator it = models.begin(); it != models.end(); ++it) {
					...
					// Faces
					for (size_t i = 0u; i < it->indices.size(); i += 3)
