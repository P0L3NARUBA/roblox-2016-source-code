...
#define GFXSPATIALHASH_MAX_CELLS_PER_PRIMITIVE 8
		...
		const float SpatialHashedScene::maxAdmissibleVolume = pow((float)(8 << GFXSPATIALHASHLEVELS), 3.0f);
		...
		uint32_t SpatialHashedScene::globalFrameNumber = 0u;
		...
		struct FrustumVisitor : public GfxSpatialHash::SpaceFilter {
			...
			FrustumVisitor(std::vector<CullableSceneNode*>& nodes, const RenderCamera& camera, const Vector3& pointOfInterest, FrameRateManager* frm, uint32_t frameNumber)
				: nodes(nodes)
				, camera(camera)
		...
		void SpatialHashedScene::querySphere(std::vector<CullableSceneNode*>& nodes, const Vector3& center, float radius, uint32_t flags) {
		...
		void SpatialHashedScene::queryExtents(std::vector<CullableSceneNode*>& nodes, const Extents& extents, uint32_t flags) {
		...
		std::vector<CullableSceneNode*> SpatialHashedScene::getAllNodes() const {
			struct NodeVisitor : public GfxSpatialHash::SpaceFilter {
				...
				NodeVisitor(std::vector<CullableSceneNode*>& result, uint32_t frameNumber)
					: result(result)
					, frameNumber(frameNumber)
				{
				}
		...
		uint32_t SpatialHashedScene::getNextFrame() {
