...
EnvMap::EnvMap(VisualEngine* ve): Resource(ve->getDevice())
{
    ...
    texture = device->createTexture(Texture::Type_Cube, Texture::Format_R11G11B10F, kCubemapSize, kCubemapSize, 1, 6, Texture::Usage_Renderbuffer);

    for (int i = 0; i < 6; ++i) {
        shared_ptr<Renderbuffer> rb = texture.getTexture()->getRenderbuffer(i, 0);

        fbFaces[i] = device->createFramebuffer(rb); // no depth
    }
}
