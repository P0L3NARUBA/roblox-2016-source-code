	...
	namespace Graphics
	{
		...
		void ShaderManager::loadShaders(const std::string& folder, const std::string& language, bool consoleOutput)
		{
			...
			for (Value::ValueIterator it = root.Begin(); it != root.End(); ++it)
			{
				...
				bool isPixel = target.GetString()[0] == 'p';
				bool isCompute = target.GetString()[0] == 'c';
				bool isGeometry = target.GetString()[0] == 'g';
				...
				try
				{
					...
					if (isVertex) {
						...
						if (reloadShaders && computeShaders.count(name.GetString())) {
							...
							computeShaders[name.GetString()] = shader;
						}
					}
					else if (isGeometry) {
						if (reloadShaders && geometryShaders.count(name.GetString())) {
							...
							geometryShaders[name.GetString()] = shader;
						}
					}
				}
