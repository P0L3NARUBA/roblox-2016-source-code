...
namespace G3D {
	...
	void BinaryInput::readBool8(std::vector<bool>& out, int64_t n) {
		out.resize((int32_t)n);
	...
	void BinaryInput::readBool8(Array<bool>& out, int64_t n) {
		out.resize(n);
...
void BinaryInput::read##ucase(std::vector<lcase>& out, int64_t n) {\
    out.resize(n);\
    read##ucase(&out[0], n);\
}\
\
\
void BinaryInput::read##ucase(Array<lcase>& out, int64_t n) {\
    out.resize(n);\
    read##ucase(out.begin(), n);\
}


	IMPLEMENT_READER(UInt8, uint8_t)
		IMPLEMENT_READER(Int8, int8_t)
		IMPLEMENT_READER(UInt16, uint16_t)
		IMPLEMENT_READER(Int16, int16_t)
		IMPLEMENT_READER(UInt32, uint32_t)
		IMPLEMENT_READER(Int32, int32_t)
		IMPLEMENT_READER(UInt64, uint64_t)
		IMPLEMENT_READER(Int64, int64_t)
		IMPLEMENT_READER(Float32, float)
		IMPLEMENT_READER(Float64, double)
...
#define IMPLEMENT_READER(ucase, lcase)\
void BinaryInput::read##ucase(lcase* out, int64_t n) {\
    if (sizeof(lcase) == 1) {\
        readBytes(out, n);\
    } else {\
        for (int64_t i = 0; i < n ; ++i) {\
...
#undef IMPLEMENT_READER
		...
		void BinaryInput::loadIntoMemory(int64_t startPosition, int64_t minLength) {
	...
	static uint32_t readUInt32(const uint8_t* data, bool swapBytes) {
		if (swapBytes) {
			uint8_t out[4] = {};
			out[0] = data[3];
			out[1] = data[2];
	...
	BinaryInput::BinaryInput(
		...
		if (compressed) {
			...
			m_buffer = (uint8_t*)malloc(m_length); // was: alignedMalloc()

			uint32_t L = m_length;
			// Decompress with zlib
			int64_t result = uncompress(m_buffer, (unsigned long*)&L, data + 4, dataLen - 4);
			m_length = L;
			m_bufferLength = L;
	...
	void BinaryInput::readBytes(void* bytes, int64_t n) {
		prepareToRead(n);
	...
	uint64 BinaryInput::readUInt64() {
		...
		if (m_swapBytes) {
			...
			*(uint64_t*)out = *(uint64_t*)(m_buffer + m_pos);
		}
	...
	std::string BinaryInput::readString(int64_t n) {
		assert(false); // we should not be using this
	...
	uint32_t BinaryInput::readBits(size_t numBits) {
		debugAssert(m_beginEndBits == 1);

		uint32_t out = 0u;

		const size_t total = numBits;
		while (numBits > 0) {
			if (m_bitPos > 7) {
