...
namespace G3D {
	...
	Color3 Color3::ansiMap(uint32_t i) {
	...
	Color3 Color3::pastelMap(uint32_t i) {
		uint32_t x = Crypto::crc32(&i, sizeof(uint32_t));
	...
	const Color3& Color3::purple() {
		static Color3 c(0.5f, 0.0f, 1.0f);
		return c;
	}


	const Color3& Color3::cyan() {
		static Color3 c(0.0f, 0.5f, 1.0f);
		return c;
	}
	...
	const Color3& Color3::wheelRandom() {
		...
		return colorArray[random.integer(0, 7)];
	}


	size_t Color3::hashCode() const {
		uint32_t rhash = (*(int32_t*)(void*)(&r));
		uint32_t ghash = (*(int32_t*)(void*)(&g));
		uint32_t bhash = (*(int32_t*)(void*)(&b));

		return rhash + (ghash * 37u) + (bhash * 101u);
	}
	...
	Color3 Color3::fromARGB(uint32_t x) {
	...
	Color3 Color3::fromHSV(const Vector3& _hsv) {
		...
		const int32_t i = G3D::min(5, G3D::iFloor(6.0 * _hsv.x));
