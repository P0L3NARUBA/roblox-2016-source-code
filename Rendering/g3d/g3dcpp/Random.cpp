...
namespace G3D {
	...
	Random::Random(uint32_t seed) {
		const uint32_t X = 1812433253UL;

		state = new uint32_t[N];
		state[0] = seed;
		for (index = 1; index < (int32_t)N; ++index) {
	...
	uint32_t Random::bits() {
		...
		if ((uint32_t)localIndex >= (uint32_t)N) {
			generate();
			localIndex = 0;
		}
	...
	uint32_t Random::integer(uint32_t low, uint32_t high) {
		uint32_t r = iFloor(low + (high - low + 1u) * (double)bits() / 0xFFFFFFFFUL);
	...
	int32_t Random::integer(int32_t low, int32_t high) {
		int32_t r = iFloor(low + (high - low + 1) * (double)bits() / 0xFFFFFFFFUL);
