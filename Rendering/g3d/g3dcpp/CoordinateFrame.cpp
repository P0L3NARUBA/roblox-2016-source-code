...
namespace G3D {
	...
	CoordinateFrame CoordinateFrame::fromXYZYPRRadians(float x, float y, float z, float yaw,
		...
		rotation = Matrix3::fromAxisAngleFast(rotation.column(0u), pitch) * rotation;
		rotation = Matrix3::fromAxisAngleFast(rotation.column(2u), roll) * rotation;
	...
	void CoordinateFrame::getXYZYPRRadians(float& x, float& y, float& z,
		...
		if (abs(look.y) > 0.99f) {
			...
			roll = 0.0f;//acos(actualRight.dot(expectedRight));  TODO
		}
	}
	...
	CoordinateFrame CoordinateFrame::lerp(
		...
		if (alpha == 1.0f) {
			...
			return CoordinateFrame(
				...
				translation * (1.0f - alpha) + other.translation * alpha);
		}
	}
