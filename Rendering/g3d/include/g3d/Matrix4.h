...
namespace G3D {
	...
	private:
		...
		float subDeterminant(size_t excludeRow, size_t excludeCol) const;
	...
	public:
		...
		/* a is the upper left 3x3 submatrix and b is the lower left 1x3 submatrix. The last column of the created matrix is (0, 0, 0, 1). */
		Matrix4(const class Matrix3& upper3x3, const class Vector3& lastRow = Vector3::zero());
		...
		void setRow(uint32_t r, const class Vector4& v);
		void setColumn(uint32_t c, const Vector4& v);

		const Vector4& row(uint32_t r) const;
		Vector4 column(uint32_t c) const;
