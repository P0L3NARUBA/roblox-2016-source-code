 ...
 */
	...
	private:
		...
		/** Ensures that we are able to read at least minLength from startPosition (relative
			to start of file). */
		void loadIntoMemory(int64_t startPosition, int64_t minLength = 0);

		/** Verifies that at least this number of bytes can be read.*/
		inline void prepareToRead(int64_t nbytes) {
	...
	public:
		...
		*/
		 ...
		 */
		inline uint8_t operator[](int64_t n) {
			setPosition(n);
			return readUInt8();
		}
		...
		uint16_t inline readUInt16() {
			...
			if (m_swapBytes) {
				...
				return *(uint16_t*)(&m_buffer[m_pos - 2]);
#else
				uint8_t out[2];
...
#ifdef G3D_ALLOW_UNALIGNED_WRITES
				return *(uint32_t*)(&m_buffer[m_pos - 4]);
#else
				uint8_t out[4];
		...
		void readBytes(void* bytes, int64_t n);
...
#   define DECLARE_READER(ucase, lcase)\
    ...
    void read##ucase(Array<lcase>& out, int64 n);
		...
		DECLARE_READER(UInt8, uint8_t)
		DECLARE_READER(Int8, int8_t)
		DECLARE_READER(UInt16, uint16_t)
		DECLARE_READER(Int16, int16_t)
		DECLARE_READER(UInt32, uint32_t)
		DECLARE_READER(Int32, int32_t)
		DECLARE_READER(UInt64, uint64_t)
		DECLARE_READER(Int64, int64_t)
		DECLARE_READER(Float32, float)
		DECLARE_READER(Float64, double)
