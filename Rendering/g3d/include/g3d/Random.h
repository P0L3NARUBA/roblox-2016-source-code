...
namespace G3D {
	...
	public:
		...
		Random(uint32_t seed = 0xF018A4D2);

		virtual ~Random();

		/** Each bit is random.  Subclasses can choose to override just
		   this method and the other methods will all work automatically. */
		virtual uint32_t bits();

		/** Uniform random unsigned integer on the range [min, max] */
		virtual uint32_t integer(uint32_t min, uint32_t max);

		/** Uniform random integer on the range [min, max] */
		virtual int32_t integer(int32_t min, int32_t max);
