...
namespace RBX {
	...
	RbxCamera::RbxCamera() {
		nearPlane = (float)inf();
		farPlane = 0.1f;
		setFieldOfView((float)G3D::toRadians(90.0f));
	}
	...
	void RbxCamera::setFieldOfView(float angle) {
		debugAssert((angle < G3D::pi()) && (angle > 0.0f));
	...
	RBX::RbxRay RbxCamera::worldRay(
		...
		uint32_t screenWidth = G3D::iFloor(viewport.width());
		uint32_t screenHeight = G3D::iFloor(viewport.height());
	...
	Vector3 RbxCamera::project(
		...
		uint32_t screenWidth = (uint32_t)viewport.width();
		uint32_t screenHeight = (uint32_t)viewport.height();
		...
		if (w <= 0.0f) {
			...
			// (helps with clipping)
			return Vector3(((out.x < 0.0f) ? -std::numeric_limits<float>::infinity() : std::numeric_limits<float>::infinity()),
				((out.y > 0.0f) ? -std::numeric_limits<float>::infinity() : std::numeric_limits<float>::infinity()),
	...
	*/
	...
	void RbxCamera::frustum(const G3D::Rect2D& viewport, Frustum& fr) const {
		...
		const float x = getViewportWidth(viewport) / 2.0f;
		const float y = getViewportHeight(viewport) / 2.0f;
		...
		// we need to negate them to produce actual z values.
		face.plane = Plane(Vector3(0.0f, 0.0f, -1.0f), Vector3(0.0f, 0.0f, -nearPlane));
		...
		// Right plane
		face.plane = Plane(Vector3(-cosf(fovx / 2.0f), 0.0f, -sinf(fovx / 2.0f)), Vector3::zero());
		...
		// Left plane
		face.plane = Plane(Vector3(-fr.faceArray.last().plane.normal().x, 0.0f, fr.faceArray.last().plane.normal().z), Vector3::zero());
		...
		// Far plane
		if (farPlane < inf()) {
			face.plane = Plane(Vector3(0.0f, 0.0f, 1.0f), Vector3(0.0f, 0.0f, -farPlane));
	...
	bool RbxCamera::Frustum::containsPoint(const Vector3& point) const {
		for (size_t i = 0u; i < 6u; ++i) {			// ignore front, back
