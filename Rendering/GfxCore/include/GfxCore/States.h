...
namespace RBX {
	namespace Graphics {
		...
		public:
			...
			RasterizerState(CullMode cullMode, int32_t depthBias = 0)
				: cullMode(cullMode)
				, depthBias(depthBias)
			{
			}
			...
			int32_t getDepthBias() const { return depthBias; }
		...
		public:
			...
			BlendState(Mode mode, uint32_t colorMask = Color_All) {
				struct Translation {
					Factor src;
					Factor dst;
				};
			...
			BlendState(Factor colorSrcIn, Factor colorDstIn, Factor alphaSrcIn, Factor alphaDstIn, uint32_t colorMask = Color_All) {
				construct(colorSrcIn, colorDstIn, alphaSrcIn, alphaDstIn, colorMask);
			}

			BlendState(Factor src, Factor dst, uint32_t colorMask = Color_All) {
			...
			uint32_t getColorMask() const { return colorMask; }

			uint32_t getColorSrc() const { return colorSrc; }
			uint32_t getColorDst() const { return colorDst; }
			uint32_t getAlphaSrc() const { return alphaSrc; }
			uint32_t getAlphaDst() const { return alphaDst; }
		...
		private:
			...
			void construct(Factor colorSrcIn, Factor colorDstIn, Factor alphaSrcIn, Factor alphaDstIn, uint32_t colorMask) {
		...
		public:
			...
			SamplerState(Filter filter, Address address = Address_Wrap, uint32_t anisotropy = 0u)
				: filter(filter)
				, address(address)
			...
			uint32_t getAnisotropy() const { return anisotropy; }
