...
namespace RBX {
		...
		const G3D::Color4& largeGridColor)
	{
		...
		if (camera.frustum().intersectsSphere(Vector3::zero(), ArrowSize))
		{
			...
			adorn->setObjectToWorldMatrix(CoordinateFrame(Vector3(0.0f, 0.0f, 0.0f)));
			adorn->ray(RBX::RbxRay(Vector3(0.0f, 0.0f, 0.0f), Vector3(ArrowSize, 0.0f, 0.0f)), axisColors[0]);
			adorn->ray(RBX::RbxRay(Vector3(0.0f, 0.0f, 0.0f), Vector3(0.0f, ArrowSize, 0.0f)), axisColors[1]);
			adorn->ray(RBX::RbxRay(Vector3(0.0f, 0.0f, 0.0f), Vector3(0.0f, 0.0f, ArrowSize)), axisColors[2]);
		...
		int							boxesPerStud)
	{
		...
		for (int i = xMinInt; i <= xMaxInt; i++)
		{
			...
			cylinder(adorn, adornCoord, direction1, (float)(yMaxInt - yMinInt), 0.03f, color);
			if (i < xMaxInt)
			{
				for (int j = 1; j < boxesPerStud; j++)
				{
					...
					adornCoord.translation = ptInWorld;
					cylinder(adorn, adornCoord, direction1, (float)(yMaxInt - yMinInt), 0.01f, color);
				}
			}
		}
		...
		for (int i = yMinInt; i <= yMaxInt; i++)
		{
			...
			adornCoord.translation = ptInWorld;
			cylinder(adorn, adornCoord, direction2, (float)(xMaxInt - xMinInt), 0.03f, color);
			if (i < yMaxInt)
			{
				for (int j = 1; j < boxesPerStud; j++)
				{
					...
					adornCoord.translation = ptInWorld;
					cylinder(adorn, adornCoord, direction2, (float)(xMaxInt - xMinInt), 0.01f, color);
				}
			}
		}
	}
	...
	void DrawAdorn::axisWidget(Adorn* adorn, const RBX::Camera& camera)
	{
		...
		Vector3 offset = Vector3((viewport.width() / 2.0f) - 50.0f, (-viewport.height() / 2.0f) + 50.0f, 0.0f);
		Vector3 widgetOrigin3dPos = camera.getRenderingCoordinateFrame().translation + (camera.getRenderingCoordinateFrame().lookVector() * 2.0f);
