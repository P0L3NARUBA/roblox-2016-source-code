...
namespace RBX {
	...
	static const uint32_t AveragingFrames = 40u;
	static const uint32_t VarianceFrames = 20u;

	static const uint32_t LockStepDelayDown = 100u;		// Number of frames to wait after going a quality level down
	static const uint32_t LockStepDelayUp = 150u;		// Number of frames to wait after going a quality level up
	static const double RenderFraction = 0.625;
	static const double VarianceLimit = 5.0;

	static const double MultiCoreRenderBottleneckFraction = 0.8;
	static const double MultiCorePrepareFraction = 0.3;

	static const uint32_t SwitchCounterMax = 10u;

	static const uint32_t SettleDelay = 20u; // Number of milliseconds that we consider level stable
	...
	static const double FastBackoffMaxFrameLen = 40.0;	// 25 FPS
#else
	static const double FastBackoffMaxFrameLen = 60.0;	// 16.6 FPS
#endif

	static const uint32_t FastBackoffFPSAve = 10u;			// frames
	static const uint32_t FastBackoffWatchingFrames = 5u;		// frames

	static const uint32_t SqDistanceBump = 50u;
	...
	struct THROTTLE_LOCKSTEP {
		...
		uint32_t textureAnisotropy;
		SSAOLevel ssao;
		float lightGridRadius;
		bool lightAllowNonFixed;
		size_t lightChunkBudget;
		uint32_t throttlingFactor; // Matches physics throttling table in World.cpp: - 0/8, 1/8, 1/4, 1/3, 1/2, 2/3, 3/4, 7/8, 15/16
		double StepHill;
		double MaxStepHill;
	};
	...
	FrameRateManager::FrameRateManager(void) :
		...
		mStableFramesCounter(0u),
		mThrottlingOn(false),
		mCurrentQualityLevel(0u),
		...
		mIsStable(false),
		mBlockCounter(0u),
		mLastBlockCounter(0u),
		mAdjustmentOn(true),
		mBadBackoffFrameCounter(0u),
		...
		mAggressivePerformance(false)
	{
		...
		for (uint32_t i = 0u; i < CRenderSettings::QualityLevelMax; ++i)
			mQualityCount[i] = 0u;
	...
	float FrameRateManager::GetTargetFrameTime(uint32_t level) const
	{
		return mAggressivePerformance ? 19.0f : LockstepTable[level].framerate;
	}

	void FrameRateManager::AddBlockQuota(uint32_t blocksInCluster, float sqDistanceToCamera, bool isInSpatialHash)
	{
	...
	void FrameRateManager::UpdateStats(double frameTime, double renderTime, double prepareTime) {
		...
		if (mCurrentQualityLevel > 0u) {
			// prevent overflow (really unlike, but still)
			if (mQualityCount[mCurrentQualityLevel] == UINT_MAX - 1u)
				for (uint32_t i = 0u; i < CRenderSettings::QualityLevelMax; ++i)
					mQualityCount[i] /= 2u;
	...
	float FrameRateManager::GetTargetFrameTimeForNextLevel() const {
		RBXASSERT(mCurrentQualityLevel < (CRenderSettings::QualityLevelMax - 1u));

		return GetTargetFrameTime(mCurrentQualityLevel + 1u);
	}

	float FrameRateManager::GetTargetRenderTimeForNextLevel() const {
		RBXASSERT(mCurrentQualityLevel < (CRenderSettings::QualityLevelMax - 1u));

		return GetTargetFrameTime(mCurrentQualityLevel + 1u) * MultiCoreRenderBottleneckFraction - LockstepTable[mCurrentQualityLevel + 1].StepHill;
	}
	...
	void FrameRateManager::AdjustQuality(double frameTime, double renderTime, bool adjustmentOn, double bonusTime) {
		...
		if (mQualityDelayDown > 0u)
			mQualityDelayDown--;

		if (mQualityDelayUp > 0u)
			mQualityDelayUp--;
		...
		if (mQualityDelayDown > 0u && mQualityDelayUp > 0u)
			return;
		...
		if ((mQualityDelayDown == 0u) && (mCurrentQualityLevel > 1u)
			&& (frameStats.average > GetTargetFrameTime(mCurrentQualityLevel)) && bRenderLimited)
			StepQuality(false, false);

		// Check for going up:
		else if ((mQualityDelayUp == 0u) && (mCurrentQualityLevel < (CRenderSettings::QualityLevelMax - 1u))
			&& frameStats.average < GetTargetFrameTimeForNextLevel())
		{
			...
			if (RBX::TaskScheduler::singleton().getThreadCount() > 1u)
			{
				renderingHasRoom = (renderStats.average < GetTargetRenderTimeForNextLevel()) &&
					prepareStats.average < GetTargetFrameTime(mCurrentQualityLevel + 1u) * MultiCorePrepareFraction;
			}
	...
	void FrameRateManager::StepQuality(bool stepUp, bool isBackOff) {
		uint32_t oldQualityLevel = mCurrentQualityLevel;

		mCurrentQualityLevel = stepUp ? mCurrentQualityLevel + 1u : mCurrentQualityLevel - 1u;
		...
		if (mCurrentQualityLevel > 1u) {
			...
			if (stepUp != mWasQualityUp && mSwitchCounter < SwitchCounterMax) {
				...
				if (!stepUp) {
					...
					uint32_t previousLevel = mCurrentQualityLevel + 1u;

					double StepHillAdd = isBackOff ? 0.1 : 1.0;
	...
	double FrameRateManager::getMetricValue(const std::string& metric) {
		if (metric == "FRM")
			...
			if (mSettings->getQualityLevel() != CRenderSettings::QualityAuto)
				...
				return mCurrentQualityLevel + 1u < CRenderSettings::QualityLevelMax ? LockstepTable[mCurrentQualityLevel + 1].StepHill : 0.0;
		}
	...
	uint32_t FrameRateManager::getPhysicsThrottling() const {
	...
	uint32_t FrameRateManager::getTextureAnisotropy() const {
	...
	size_t FrameRateManager::getLightingChunkBudget() const {
